...
def get_currency_balance( symbol, contract ):
    ...
    cmd = "cleos -u " + endpoint + " get currency balance " + contract + " " + account_name + " " + symbol
...
with open('./balances.txt', 'wb') as f:
    ...
    for key in asset_dict.keys():
        ...
        cmd = "cleos -u https://mainnet.eoscanada.com get currency balance " + asset_dict[key] + " " + account_name + " " + key.lower()
        asset_balance = os.popen(cmd).read().split(" ")[0]
        if asset_balance == "": 
            asset_balance = 0
    ...
    cmd = "cleos -u https://mainnet.eoscanada.com get currency balance bitpietokens " + account_name + " EUSD"
    asset_balance = os.popen(cmd).read().split(" ")[0]
    if asset_balance == "":
        asset_balance = 0
