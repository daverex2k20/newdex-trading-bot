...
#Unlock wallet
cmd="cleos wallet unlock --password PW5JSQ2nzdPGMVeob51henFd8fn6QujpBJ8TynM36ifPTTCGeXPTs"
...
#			(alloted_EUSD) - ( amount of asset left in sells * price ) )
    ...
    if sell_amt > 0.005:  # Even for BTC this is small.
        ...
        cmd='cleos -u https://api.cypherglass.com transfer -c ' + asset_dict[key] + ' guztgmbsgige newdexpocket ' + '"'+str('%.4f' % sell_amt)+' '+key+'" \''+memo+'\''
        print cmd
        print os.popen(cmd).read()
    ...
    if buy_amt_eusd > 1.0:  # Greater than 1 dollar
        ...
        cmd='cleos -u https://api.cypherglass.com transfer -c bitpietokens guztgmbsgige newdexpocket ' + '"'+str('%.4f' % buy_amt_eusd)+' EUSD" \''+memo+'\''
        print cmd
        print os.popen(cmd).read()
