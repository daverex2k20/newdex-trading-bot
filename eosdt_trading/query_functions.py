...
def get_newdex_open_orders( symbol ):
    ...
    api_key='uj1h2rv7un4ndyos'
    secret_key='givgmugonwguw79fcfiflk87ao959ebp'
...
def get_currency_balance( symbol, contract ):
    ...
    cmd = "cleos -u " + endpoint + " get currency balance " + contract + " guztgmbsgige " + symbol
...
with open('./balances.txt', 'wb') as f:
    ...
    for key in asset_dict.keys():
        ...
        cmd = "cleos -u https://mainnet.eoscanada.com get currency balance " + asset_dict[key] + " guztgmbsgige " + key.lower()
        asset_balance = os.popen(cmd).read().split(" ")[0]
        if asset_balance == "": 
            asset_balance = 0
    ...
    cmd = "cleos -u https://mainnet.eoscanada.com get currency balance bitpietokens guztgmbsgige EUSD"
    asset_balance = os.popen(cmd).read().split(" ")[0]
    if asset_balance == "":
        asset_balance = 0
