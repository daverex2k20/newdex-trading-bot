...
with open('./balances.txt', 'w') as f:
    ...
    for key in asset_dict.keys():
        ...
        cmd = "cleos -u https://api.eos.miami get currency balance " + asset_dict[key] + " " + account_name + " " + key.lower()
        ...
        symbol = asset_dict[key] +  "-" + key.lower() + "-tlos"

        timestamp = str(int(time.time()))
        
        message = 'api_key=' + api_key + '&symbol=' + symbol + '&state=pending&timestamp=' + timestamp
        
        utf8message = bytes(message, encoding='utf-8')
        utf8secret_key = bytes(secret_key, encoding='utf-8')
        signature = hmac.new(key=utf8secret_key, msg=utf8message, digestmod=hashlib.sha256).hexdigest()
        ...
        for i in range(len(data["data"])):
            ...
            if (order["state"] == "new" or order["state"] == "partially-filled"):
                ...
                if int(timestamp) - int(order["updated_at"]) > 172800: # 2 days
                    ...
                    url='cleos -u https://api.eos.miami get transaction ' + order["trx_id"]
...
#    cmd = "cleos -u https://api.eos.miami get currency balance bitpietokens " + account_name
#    asset_balance = os.popen(cmd).read().split(" ")[0]
#    if asset_balance == "":
#        asset_balance = 0
